...
zones.append({"rect": pygame.Rect(100, 100, 150, 75), "type": "add_tool"})
zones.append({"rect": pygame.Rect(100, 200, 150, 75), "type": "search_tool"})
...
while running:
            ...
            for zone in zones:
                ...
                if rect.collidepoint(image_x, image_y):
                    ...
                    if ztype == "photo" or ztype == "pdf":
                        ...
                        print(f"[Karen] Playing video with audio: {path}")
                        subprocess.Popen([
                            "ffplay",
                            "-noborder",
                            "-x", str(LIVE_FEED_RECT.width),
                            "-y", str(LIVE_FEED_RECT.height),
                            "-window_title", "KarenMemory",
                            "-autoexit",
                            "-loglevel", "quiet",
                            path
                        ])
    ...
    # Draw labeled buttons for "Add Tool" and "Search Tool"
    ...
    for zone in zones:
        ztype = zone.get("type")
        if ztype in ("add_tool", "search_tool"):
            rect = zone["rect"]
            pygame.draw.rect(screen, button_color, rect)
            label = "Add Tool" if ztype == "add_tool" else "Search Tool"
