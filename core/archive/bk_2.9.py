...
class InteractiveZone(pygame.sprite.Sprite):
	...
	def draw_zone(self):
		...
		if self.hovered:
			border_color = (0, 255, 0)  # Green RGB (no alpha)
	...
	def update_hover(self, cursor_rect):
		...
		adjusted_cursor_rect = cursor_rect
		was_hovered = self.hovered
		self.hovered = self.rect.colliderect(adjusted_cursor_rect)
		if self.hovered != was_hovered:
			self.draw_zone()

	def on_click(self, current_frame, scroll_x):
		adjusted_cursor_x = (screen_width // 2) + scroll_x
		adjusted_cursor_rect = pygame.Rect(adjusted_cursor_x - 5, cursor_y - 5, 10, 10)
		if self.rect.colliderect(adjusted_cursor_rect):
			...
			if self.type == "snapshot":
				...
				cv2.imwrite(os.path.join("snapshots", filename), cv2.cvtColor(current_frame, cv2.COLOR_RGB2BGR))
				print(f"[Karen] Snapshot saved: {filename}")

# Database functions
def connect_db():
	return sqlite3.connect("/home/lynnsanity/Karen/garage.db")
...
class ZoneEditDialog:
	...
	def draw(self, screen):
		...
		prompt = self.font.render("Press Enter to switch field, Spacebar to save", True, (200, 200, 200))
...
image_path = "/home/lynnsanity/Karen/Photos/GARAGE.jpg"
...
print(f"[Karen] Webcam resolution set to: {int(actual_width)}x{int(actual_height)}")
...
threading.Thread(target=webcam_capture_loop, daemon=True).start()

running = True
while running:
	with frame_lock:
		if current_frame is None:
			continue  # Skip frame if not ready yet
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			...
			for zone in zones:
				if zone.type == "snapshot":
					zone.on_click(frame, scroll_x)
		elif confirm_dialog:
	...
	cursor_rect = pygame.Rect(screen_width // 2 - 5 + scroll_x, cursor_y - 5, 10, 10)
	...
	for zone in zones:
		...
		if zone.type == "live_feed":
			scaled_frame = cv2.resize(current_frame, (zone.rect.width, zone.rect.height))
			frame_surface = pygame.image.frombuffer(scaled_frame.tobytes(), scaled_frame.shape[1::-1], "RGB")
			screen.blit(frame_surface, (zone_x, zone.rect.y))
